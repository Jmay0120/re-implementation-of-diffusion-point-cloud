...
cate_to_synsetid = {v: k for k, v in synsetid_to_cate.items()}  # 建立类别到ID的映射
...
class ShapeNetCore(Dataset):
    ...
    def __init__(self, path, cates, split, scale_mode, transform=None):
        ...
        self.path = path
        if 'all' in cates:
            cates = cate_to_synsetid.keys()  # 获取所有类别
        self.cate_synsetids = [cate_to_synsetid[s] for s in cates]  #获取对应的类别ID
    ...
    def get_statistics(self):
        ...
        basename = os.path.basename(self.path)  # 提取文件名
        dsetname = basename[:basename.rfind('.')]  # 去掉文件扩展名
        stats_dir = os.path.join(os.path.dirname(self.path), dsetname + '_stats')  # 拼接出一个新目录路径，用于保存统计信息
        os.makedirs(stats_dir, exist_ok=True)

        if len(self.cate_synsetids) == len(cate_to_synsetid):
            stats_save_path = os.path.join(stats_dir, 'stats_all.pt')  # 点云数据信息保存路径
        else:
        ...
        mean = all_points.view(B*N, -1).mean(dim=0) # (1, 3)  各坐标的平均值
        std = all_points.view(-1).std(dim=0)        # (1, )  所有数值的总体标准差
